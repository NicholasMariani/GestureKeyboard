package com.admin.keyboard.kbd;
...
import com.admin.keyboard.kbd.Keyboard.Key;
...
public class KeyboardView extends View implements View.OnClickListener {
	...
	public interface OnKeyboardActionListener {
		...
		/**
		 * Called when the user moves accross the keyboard. This is sent between {@link #onPress} and {@link #onKey}.
		 *
		 * @param primaryCode the unicode of the key being pressed. If the touch is not on a valid
		 *                    key, the value will be zero.
		 */
		void onSwipe(int primaryCode);
	...
	private List<String> action_keys = new LinkedList<>(Arrays.asList("Shift", "Del", "Ctrl", "?123", "Send", "ABC", "1/2", "2/2"));
	private boolean mPossiblePoly;

	protected SwipeTracker mSwipeTracker = new SwipeTracker();
	...
	private Drawable mActionKeyBackground;
	private Drawable mNumKeyBackground;
	private Drawable mCapsLockBackground;
	private Drawable mKeyBackground;

	private Drawable mKeyBackgroundPressed;
	...
	public KeyboardView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		for (int i = 0; i < n; i++) {
			...
			} else if (attr == R.styleable.KeyboardView_actionKeyBackground) {
				mActionKeyBackground = a.getDrawable(attr);
			} else if (attr == R.styleable.KeyboardView_numKeyBackground) {
				mNumKeyBackground = a.getDrawable(attr);
			} else if (attr == R.styleable.KeyboardView_capsLockBackground) {
				mCapsLockBackground = a.getDrawable(attr);
			} else if (attr == R.styleable.KeyboardView_keyBackgroundPressed) {
				mKeyBackgroundPressed = a.getDrawable(attr);
...
//		mPredicting = true;
		...
		mActionKeyBackground.getPadding(mPadding);
		mNumKeyBackground.getPadding(mPadding);
		mCapsLockBackground.getPadding(mPadding);
		mKeyBackgroundPressed.getPadding(mPadding);
	...
	private void onBufferDraw() {
		...
		for (int i = 0; i < keyCount; i++) {
			...
			Drawable tempKeyBackground = null;
			int[] drawableState = key.getCurrentDrawableState();
			for(int j = 0; j < drawableState.length; j++) {
				if(drawableState[j] == android.R.attr.state_pressed)
					tempKeyBackground = mKeyBackgroundPressed;
			}
			if(tempKeyBackground == null && (action_keys.contains(key.label.toString()) ||
					(key.label.equals("") && key.keyMode == 2)))
				tempKeyBackground = mActionKeyBackground;
			else if(tempKeyBackground == null)
				tempKeyBackground = mKeyBackground;

			if(key.label.toString().equals("Shift") && getCapsLock()) {
				tempKeyBackground = mCapsLockBackground;
			}

			if(key.label.chars().allMatch(Character::isDigit) && !key.label.toString().isEmpty()) {
				tempKeyBackground = mNumKeyBackground;
			}

			tempKeyBackground.setState(drawableState);

			// Switch the character to uppercase if shift is pressed
			String label = key.label == null ? null : adjustCase(key.label).toString();

			final Rect bounds = tempKeyBackground.getBounds();
			if (key.width != bounds.right ||
					key.height != bounds.bottom) {
				tempKeyBackground.setBounds(0, 0, key.width, key.height);
			}
			canvas.translate(key.x + kbdPaddingLeft, key.y + kbdPaddingTop);
			tempKeyBackground.draw(canvas);
		...
		if (oldKeyIndex != mCurrentKeyIndex && mShowPreview) {
			...
			if (previewPopup.isShowing()) {
				if (keyIndex == NOT_A_KEY) {
					mHandler.sendMessage(mHandler.obtainMessage(MSG_REMOVE_PREVIEW));
				}
			}
			if (keyIndex != NOT_A_KEY) {
	...
	private void showKey(final int keyIndex) {
		...
		if(!action_keys.contains(key.label.toString()) && key.codes[0] != 32) {
			...
			if (mMiniKeyboardContainer == null) {
				...
				mMiniKeyboard.setOnKeyboardActionListener(new OnKeyboardActionListener() {
					...
					public void onSwipe(int primaryCode) {
						mKeyboardActionListener.onSwipe(primaryCode);
					}
	...
	private boolean onModifiedTouchEvent(MotionEvent me, boolean possiblePoly) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				mKeyboardActionListener.onSwipe(keyIndex != NOT_A_KEY ?
						mKeys[keyIndex].codes[0] : 0);
	...
	protected void setSwipeActive(boolean isActive) {
		swipeActive = isActive;
	}

	public boolean getSwipeActive() {
		return swipeActive;
	}
	...
	public SwipeTracker getmSwipeTracker() {
		return mSwipeTracker;
	}

	protected static class SwipeTracker {

		public static final int NUM_PAST = 50;
		public static final int LONGEST_PAST_TIME = 200;

		public float mPastX[] = new float[NUM_PAST];
		public float mPastY[] = new float[NUM_PAST];
		public long mPastTime[] = new long[NUM_PAST];

		public float mYVelocity;
		public float mXVelocity;

		public void clear() {
			mPastX = new float[NUM_PAST];
			mPastY = new float[NUM_PAST];
			mPastTime = new long[NUM_PAST];
		}
